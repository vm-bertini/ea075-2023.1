...
### Debounce
 ...
 A fim de evitar que o sistema entenda que apertamos o botão mais de uma vez quando apertamos somente uma vez ou outro ruídos semelhantes nomeados de bouce, usaremos o debouce. Para isso utilizaremos um circuito formado por um capacitor e duas resistência como na imagem abaixo.
...
O algoritmo utilizado controlará os três módulos acima e todos módulos necessitam da função de temporização do microcontrolador para medir períodos de espera, ou seja, os intervalos entre irrigações, fertilizações ou ciclos de iluminação. Como são ciclos longos de espera (horas ou dias), optamos por utilizar um código por interrupções ao invés de varredura, para tentar minimizar o consumo de energia nos momentos em que o microcontrolador não estiver realizando nemhuma função além de temporização. 
...
A rotina de irrigação será chamada após o fim de um dos períodos de temporização (3 ou 12 horas) e deverá identificar em qual ponto do fluxograma ela está a partir da duração do intervalo que foi temporizado e seguir a execução do fluxograma do código, encerrando quando chegar em outro momento de espera.
