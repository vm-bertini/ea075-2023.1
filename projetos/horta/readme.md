...
Assim, o loop principal do código não fará nada. O temporizador será configurado para o maior prescaler possível e a cada ciclo de contagem irá gerar uma interrupção que nela três incrementa três variáveis globais do tipo int que temporizam as funções de irrigação, fertilização e iluminação. Além disso, o código terá outra variável global do tipo int com o valor de referência para saber quando a temporização de irrigação chegou ao fim, pois a irrigação precisa temporizar períodos de 3 ou 12 horas. Quando cada temporização chegar ao fim ela chama a função correspondente.
...
A rotina de irrigação será chamada após o fim de uma temporização e deverá identificar em qual ponto do fluxograma ela está a partir da duração dp intervalo que foi temporizado e seguir a execução do fluxograma do código, encerrando quando chegar em outro momento de espera.

Além disso, os botões e o interruptor também devem gerar interrupções que tratem esses eventos.

Assim, sabendo os recursos que necessário do microcontrolador é possível justificar a sua escolha. Na parte estrutural foi definico que o sistema necessita de 7 entradas de propósito geral, 2 entradas analógica e um temporizador, na parte de código foi definido que, a princípio serão utilizadas 6 variáveis globais do tipo int, de 2B cada e 2 variáveis globais do tipo char, de 1B cada. As variáveis locais de cada rotina não são definidas simultaeamente e nenhuma rotina terá muitas variáveis, logo é provável que 256B de memória SRAM sejam suficientes. Dada a familiaridade com o microcontrolados AtMega328p, optamos por escolher outro microcontrolador AVR que fosse um pouco mais simples e mais barato, optando pelo ATtiny40.
